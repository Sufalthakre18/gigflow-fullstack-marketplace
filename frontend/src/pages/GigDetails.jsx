...
export default function GigDetails() {
  ...
  const { currentGig, loading, error, getGig } = useGig();
  const { bids, createBid, getBidsForGig, hireBid, clearBids } = useBid();
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        {isOwner && bids.length > 0 && (
          <div className="mt-6 grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {bids.map((bid) => (
              <BidCard key={bid._id} bid={bid} onHire={hireBid} />
            ))}
          </div>
        )}
      </div>
